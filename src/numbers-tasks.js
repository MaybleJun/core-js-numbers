...
function getSumToN(n) {
  const arrN = Array.from(Array(n), (_, index) => index + 1);
  return arrN.reduce((a, b) => a + b, 0);
}
...
function getSumOfDigits(num) {
  const arrSymbol = num.toString().split('');
  return arrSymbol.reduce((a, b) => a + Number(b), 0);
}
...
function getCountOfOddNumbers(number) {
  const arrN = Array.from(Array(number), (_, index) => index + 1);
  return arrN.reduce((a, b) => a + (b % 2 !== 0 ? 1 : 0), 0);
}
